...
tuning.grid.test <- expand.grid(mtry = seq(from = 25, to = 100, by = 5),
                                mincriterion = seq(from = 0, to = 0.75, by = 0.05),
...
save(list = paste0('party.cv.output.data.partition.', job.counter.num),
     file = paste0('/home/fitzpatr/party/results/cv_results/full_grid_2/full_grid_2_party.cv.output.data.partition.', job.counter.num, '.RData')
    ) 
